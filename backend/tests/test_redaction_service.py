...
@pytest.mark.asyncio
async def test_toggle_approval(redaction_service, mock_cosmos_client):
    """Verify toggling suggestion approval."""
    mock_cosmos_client.update_item.return_value = {"approved": True}
...
@pytest.mark.asyncio
async def test_add_manual_suggestion(redaction_service, mock_cosmos_client):
    ...
    await redaction_service.add_manual_suggestion(job_id="job-1", suggestion=suggestion)
